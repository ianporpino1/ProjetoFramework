...
public interface PosicaoRepository extends JpaRepository<Posicao, Long> {
    ...
    @Query(value = "SELECT a.identificador, TRUNCATE(p.preco_medio, 2) AS preco_medio, p.quantidade_total, " +
                    "TRUNCATE(p.valor_total, 2) AS valor_total FROM posicao p JOIN ativo a ON p.id_ativo = a.id WHERE p.id_usuario = :idUsuario AND p.status_posicao=0", nativeQuery = true)
